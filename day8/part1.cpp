...
int main() {
    ...
    sort(clusters.begin(), clusters.end(), [](const unordered_set<string>& a, const unordered_set<string>& b) {
        return a.size() > b.size();
    });
    long total = clusters[0].size() * clusters[1].size() * clusters[2].size();
    cout << "Total cluster size product: " << total << endl;
